...
class AkdkrsmhsController extends FungsiController
{
    ...
    public function KrsAction($value='')
    {
    	$nomhs = $_GET['nomhs'];
		$krs_ol = $_GET['krs_ol'];
		...
		if (empty($predis->get($key_redis))) {
    		...
    		$fungsi = new PrediskrsController;
			$fungsi->DataKrsAction($nomhs,$krs_ol);			
		}
    }
	...
	public function ListMkDitawarkanAction($nomhs,$krs_ol)
    {
		...
		// }
			...
			/* data_krs_mhs */
			$data_krs_mhs = $this->RedisDataMkKrs($nomhs,$psmhs_id,$thn_akd,$session_id,$krs_ol,$thn_kur,$angkatan);

			$this->view->krs_mhs = $data_krs_mhs['krs_mhs']; // krs yang di pilih mhs
			$this->view->pilih_jadwal = $data_krs_mhs['pilih_jadwal']; // jadwal krs jam_awal - jam_akhir
			$this->view->jad_mk = $data_krs_mhs['jad_mk']; // jadwal krs jam_awal - jam_akhir
			
			$data_krs['krs_mhs'] = $data_krs_mhs['krs_mhs'];
			$data_krs['pilih_jadwal'] = $data_krs_mhs['pilih_jadwal'];
			$data_krs['jad_mk'] = $data_krs_mhs['jad_mk'];


			/* data_krs_ajukan */
			$data_krs_ajukan = $this->RedisKrsMk($nomhs,$psmhs_id,$thn_akd,$session_id,$krs_ol,$map_nilai_mk,$vcx,$thn_kur,$angkatan);
			...
			// echo "<pre>".print_r($data_krs_ajukan['list_mk_krs_diajukan'],1)."</pre>";die();

			$data_krs_ajukan['list_mk_krs_diajukan'] = $data_krs_ajukan['list_mk_krs_diajukan'];
			$data_krs_ajukan['list_prasyarat_tidak_lolos'] = $data_krs_ajukan['list_prasyarat_tidak_lolos'];

			$data_krs_count['count'] = count($data_krs_ajukan['list_mk_krs_diajukan']);
