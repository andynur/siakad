...
class AkdkrsController extends FungsiController
{
    ...
    public function ListMkDitawarkanAction($value='')
    {
		...
		if (empty($predis->get("krs_$nomhs_$krs_ol"))) {
			$nomhs = $_GET['nomhs'];
			$krs_ol = $_GET['krs_ol'];

			$key_redis = "krs_".$nomhs."_".$krs_ol;
			$predis->set($key_redis, json_encode($data));
			$predis->expire("krs_$nomhs_$krs_ol", 172800);
		}
		$this->view->pick($halaman);
    }

    public function DataKrsAction($value='')
    {
    	/* connec to redis */
    	$redis = new PredisController;
		$predis = $redis->config();
    	$data_json = $predis->get("krs_111000001_2017-1-1");
		...
		$this->view->krs_mhs = $data['krs_mhs']; // krs yang di pilih mhs
		$this->view->pilih_jadwal = $data['pilih_jadwal']; // jadwal krs jam_awal - jam_akhir
		$this->view->jad_mk = $data['jad_mk']; // jadwal krs jam_awal - jam_akhir
		$this->view->list_mk_krs_diajukan = $data['list_mk_krs_diajukan']; // list_mk_krs_diajukan
		$this->view->list_prasyarat_tidak_lolos = $data['list_prasyarat_tidak_lolos']; // list_prasyarat_tidak_lolos
